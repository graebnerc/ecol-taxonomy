here::i_am("R/experiments/clustering2.R")
library(here)
library(data.table)
library(dplyr)
library(tidyr)
...
reduced_data <- base_data %>% 
  ...
  mutate(# Here the normalization is done
    ...
    FossilShare_normed = ShareFossils_PrimEnProd, # share of fossils in primary energy production
    RenewShare_normed = ShareRenewables_PrimEnProd, # share of renewables in primary energy production
